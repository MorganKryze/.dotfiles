...
# Import color iterm2/themes for iTerm
count=0
for filename in "$@"; do
	count=$((count + 1))
	if (echo "$filename" | grep -qF '.itermcolors'); then
		filename="iterm2/themes/"$(basename "$filename")""
	fi
	if [[ ! -f "$filename" && -f "iterm2/themes/$filename.itermcolors" ]]; then
		filename="iterm2/themes/$filename.itermcolors"
	fi
	if [[ ! -f "$filename" ]]; then
